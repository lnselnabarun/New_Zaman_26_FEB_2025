 ...
 export default class StockSearch extends Component {
    ...
    onSelect = data => {
      console.log('onSelect data12: ', data)
          ...
          {
          this.setState({collectionString: data.category_name, collectionError: '', addToCart0: 'Add to Cart', showFlatList: false,
          SKUstring: 'SKU No. *', skuError: ''})
          }
          else
          {
            this.setState({SKUstring: data.category_name, skuError: '', addToCart0: 'Add to Cart', showFlatList: false})
          }
    ...
    render() {
        ...
        color:'white',
        marginLeft: 23,
        marginTop:6, fontWeight: '800', fontSize: 19}}>Stock Search</Text>
              </View>
              <TouchableOpacity
...
alignSelf: 'center',
justifyContent: 'center', marginTop: -13, marginLeft: -18}}>
 <Text style={{fontSize: 15,textAlign: 'center', color: 'black', fontWeight: 'bold'}}>{this.state.countArr}</Text></View>
...
<TouchableOpacity onPress={() => this.props.navigation.navigate('CollectionAndSKUFilter',{onSelect: this.onSelect,
                flagtoShow: 'collection', navigationFlaggg: 'stockSearch'
            })}>
<TextInput
...
<TouchableOpacity
    ...
    setQuantity = (value) =>
    {
      ...
      if (value.trim() != '')
        {
          this.setState({quantityError: '', quantityString: value, addToCart0: 'Add to Cart', showFlatList: false})
        }
        else
        {
          this.setState({quantityError: 'Please enter a valid Quantity', quantityString: value})
        }
    ...
    selectSKU = (value) =>
    {
      ...
      {
        Toast.show('Collection value must be required before choosing SKU', Toast.LONG)
        this.setState({collectionError: 'Please select Collection', })
  }
  else
  {
    this.props.navigation.navigate('CollectionAndSKUFilter',{onSelect: this.onSelect,
      flagtoShow: 'sku', navigationFlaggg: 'stockSearch'
  })
  }
    }
    backPresses = async () =>
    {
      console.log('cart item array112: ',this.state.cartArray, this.state.cartArray.length, )
      // await AsyncStorage.setItem('cartArray', JSON.stringify(this.state.cartArray))
      
      if (this.state.cartArray.length > 0)
      {
        console.log('cart item array1132: ',this.state.cartArray, this.state.cartArray.length, )
         var username = await AsyncStorage.getItem('username');
          ...
          if (myArray !== null) {
            ...
            await AsyncStorage.setItem(username+'cartArray', JSON.stringify(myArray1))
  ...
  <View style={{flexDirection:'row', alignItems: 'center', justifyContent: 'center', marginTop: 0}}>
              ...
              <View>
                <Text style={{ fontSize: 13, color: 'grey', marginLeft: 13, marginBottom: 5, fontWeight: "bold" }}>Collection</Text>
                <Text style={{ fontSize: 13, color: 'grey', marginLeft: 13, marginTop: 5, marginBottom: 5, fontWeight: "bold" }}>SKU No.</Text>
                <Text style={{ fontSize: 13, color: 'grey', marginLeft: 13, marginTop: 5, marginBottom: 5, fontWeight: "bold" }}>Stock Status</Text>
                </View>
              <View style={{ width: null, justifyContent: 'flex-end', flexDirection: 'row', marginRight: 25, marginBottom: 20 }}>
              <View>
                <Text style={{ fontSize: 13, color: 'grey', marginBottom: 5 }}>{item.collection}</Text>
                <Text style={{ fontSize: 13, color: 'grey', marginTop: 5, marginBottom: 5 }}>{item.sku}</Text>
                <Text style={{ fontSize: 13, color: 'grey', marginTop: 5, marginBottom: 5 }}>{item.stock}</Text>
                </View>
              </View>
  ...
  { this.state['addToCart'+String(index)] == 'Remove from Cart' &&  <TouchableOpacity style={{ height: 40, alignSelf: 'flex-end',
  marginTop:-8,borderRadius:5,width: 160, backgroundColor: 'green', width: 150, marginBottom: 15, marginRight: 13}}
       ...
       addToCartPressed = (status, index) => {
        ...
        if (status == 'Remove from Cart')
        {
         ...
         console.log('indexData stock: ', indexData['stock'])
          ...
          this.setState({cartArray: newIds, countArr: this.state.cartArray.length + 1})
        }
        else
        {
          var array = [...this.state.cartArray]; // make a separate copy of the array
    array.splice(index, 1);
    this.setState({cartArray: array, countArr: this.state.cartArray.length - 1});
        }
...
console.log('cart item array: ', this.state.cartArray)
       ...
       searchStockApi = async () => {
        ...
        if (this.state.collectionString.trim() == 'Collection *' && this.state.SKUstring.trim() == 'SKU No. *' && this.state.quantityString.trim() == '')
        {
          Toast.show('All fields should be required', Toast.LONG)
          this.setState({skuError: 'Please select SKU number', collectionError: 'Please select Collection', 
        ...
        else
        {
          ...
          this.setState({ collectionError: '', skuError: '', quantityError: '', itemList: []})
        var logs = {
        ...
        if (response.status === 'success') {
          ...
          if (response.StockData.collection != undefined)
          {
            ...
            this.setState({
              itemList: newIds, showFlatList: true
          ...
          Alert.alert('Status', 'Stock is not available, please check back later');
        }
